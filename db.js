...
// Added unexpected error logging to prevent ReferenceError crashes
pool.on('error', (errorWithCode) => {
    console.error('[ERROR] Unexpected database error:', errorWithCode.message);
});
...
export async function savememberAnalysis(memberInfo, analysis, researchData) {
    ...
    try {
        ...
        console.log('[INFO] Saved analysis to DB with ID ' + result.rows[0].id);
